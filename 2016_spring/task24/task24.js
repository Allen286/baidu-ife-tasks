...
function initDiv() {
    ...
    for (var i = 0; i < allDivs.length; i++) {
        addEvent(allDivs[i], 'click', function(event) {
            if (selected !== null) {
...
function addDiv() {
    var addInput = $('input')[1].value.trim();
    if (addInput === '') {
        alert('请输入要添加节点的内容');
    } else if (selected === null) {
        alert('请先选中一个节点后再点击添加按钮');
    } else {
        var chil = document.createElement('div');
        chil.innerHTML = addInput;
        selected.appendChild(chil);
        initDiv();
    }
}
